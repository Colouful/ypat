...
cd frontend-admin
pnpm install
# 本地 Docker 部署使用 docker mode，API 通过同域 /admin 反代到 system-wap
...
> 说明：
> - `frontend-admin/Dockerfile` 默认使用 `production` 模式构建；
> - `docker-compose.yml` 通过 `build.args.APP_ENV=docker` 覆盖为本地部署模式；
> - 该模式加载 `.env.docker`，`VITE_API_BASE_URL` 为空，浏览器请求会走同域 `/admin` 反代。
...
```bash
# 检查端口占用
lsof -i :3307 -i :6379 -i :8761 -i :9081 -i :8081 -i :8088 -i :5189 -i :18896
...
# ypat-admin-web Up 1min                  0.0.0.0:18896->8080/tcp
...
# 测试后台管理前端
curl -s -o /dev/null -w "%{http_code}" http://localhost:18896/
# 期望: 200

# 测试管理端登录页 API
curl -s 'http://localhost:18896/admin/captcha'
# 期望: {"code":200,"msg":"成功",...}
```
...
| 后台管理页面 | http://localhost:18896 | Vue3 管理后台 (frontend-admin) |
...
### 后台管理前端环境配置 (frontend-admin)

| 文件 | 用途 | API 地址 |
|------|------|----------|
| `.env.development` | `pnpm dev` 本地开发 | http://localhost:8081 |
| `.env.docker` | Docker 本地部署构建 | 同域 `/admin` 反代到 system-wap |
| `.env.production` | 生产构建 | https://api.ypat.com |

> 本地 Docker 部署时，`admin-web` 容器内 nginx 会把 `/admin/*` 请求转发到 `wap:8081`，浏览器无需直接访问 `8081` 端口，也无需额外处理跨域。
...
```bash
# 1. 重新构建
cd frontend-admin && pnpm exec vite build --mode docker && cd ..

# 2. admin-web 镜像需要重新构建（静态资源已打入镜像）
docker compose up --build -d admin-web
```
...
# 检查后台管理前端 nginx 配置
docker exec ypat-admin-web nginx -t
...
**解决**: 
- 确保 H5 已构建 (`npm run build:h5`)，nginx 配置中的 `proxy_pass` 指向 `wap:8081`（不是 localhost）。
- 确保后台管理前端已构建 (`pnpm exec vite build --mode docker`)，且 `admin-web` 容器已将 `/admin/*` 反代到 `wap:8081`。
...
#    - 修改端口映射（生产环境建议只暴露 8088、5189 和后台管理端口）
#    - 确保 .env.production 中的 API 地址正确
#    - 后台管理生产构建使用 `pnpm run build`（加载 `.env.production`）
...
| admin-web (后台管理) | 8080 | 18896 | 管理后台静态资源 |
