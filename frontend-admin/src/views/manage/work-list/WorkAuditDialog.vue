...
async function handleAudit(flag: string): Promise<void> {
  ...
  loading.value = true
  try {
    await auditWork(props.data.id, flag, reason.value.trim() || undefined)
...
<template>
  ...
  >
    ...
    <template #footer>
      ...
      <el-button type="danger" :loading="loading" @click="handleAudit(AuditFlag.REJECT)">
        不通过
      </el-button>
      <el-button type="primary" :loading="loading" @click="handleAudit(AuditFlag.PASS)">
        通过
      </el-button>
    </template>
  </el-dialog>
