...
CREATE TABLE `t_work_complain` (
  ...
  `pics`       VARCHAR(1000)  DEFAULT NULL                              COMMENT '投诉证据 URL，逗号分隔',
  `status`     VARCHAR(8)     NOT NULL DEFAULT '0'                     COMMENT '0待处理/1已处理/2已驳回',
  `handle_reason` VARCHAR(500) DEFAULT NULL                             COMMENT '后台处理备注',
  `created_at` DATETIME       NOT NULL DEFAULT CURRENT_TIMESTAMP       COMMENT '创建时间',
  PRIMARY KEY (`id`),
