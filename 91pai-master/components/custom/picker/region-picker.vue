  ...
  <view class="w-picker-view">
    ...
    >
      <picker-view-column>
        <view
          class="w-picker-item"
          v-for="(item,index) in range.provinces"
          :key="index"
        >{{item.label}}</view>
      </picker-view-column>
      <picker-view-column>
...
<script>
import { area_list_a } from "@/common/vmeitime-http";
  ...
  methods: {
    ...
    getcity() {
      return new Promise(async resolve => {
        let res = await area_list_a();
        if (res && res.code === 200) {
          resolve(res.res.areaData);
        } else {
          resolve([]);
        }
      });
    ...
    handlerChange(e) {
      ...
      let provinces = this.areaData;
      let citys =
        (provinces[provinceIndex] && provinces[provinceIndex].children) ||
        provinces[provinces.length - 1].children ||
        [];
      let areas = this.hideArea
        ? []
        : (citys[cityIndex] && citys[cityIndex].children) ||
          citys[citys.length - 1].children ||
          [];
      let province =
          provinces[provinceIndex] || provinces[provinces.length - 1],
