<!--
 * @Author: shawn
 * @LastEditTime: 2020-03-24 23:13:51
 -->
<template>
	<view class="tui-tabbar" :class="{'tui-tabbar-fixed':isFixed,'tui-unlined':unlined}" :style="{background:backgroundColor}">
		<block v-for="(item,index) in tabBar" :key="index">
			<view class="tui-tabbar-item" :class="{'tui-item-hump':item.hump}"
			 :style="{backgroundColor:item.hump?backgroundColor:'none'}" @tap="tabbarSwitch(index,item.hump,item.pagePath,item.verify,item.jump)">
...
<script>
	export default {
		...
		props: {
            ...
            // "jump": 'redirectTo'  跳转方式 默认 redirectTo
			tabBar: {
				type: Array,
				default () {
					return []
		...
		methods: {
			tabbarSwitch(index, hump, pagePath,verify,jump) {
				this.$emit("click", {
					index: index,
					hump: hump,
					pagePath: pagePath,
