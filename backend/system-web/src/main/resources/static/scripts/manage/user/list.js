...
var pageselectCallback = function(page_index, jq, size){
    ...
    if(currentPageData!=null){
        ...
        $.get('./list?t='+new Date().getTime(),{
            size:size,page:page_index,status:$("#status").val()
        },function(data){
...
function fillData(data){
    ...
    $.each(data,function(k,v){
        ...
        html += '<tr> ' +
            ...
            '<td>'+ (v.certcode==null?'':v.certcode) +'</td>' +
            '<td>'+ (v.statusTxt==null?'':v.statusTxt) +'</td>';
        html += '<td><a class="c-50a73f mlr-6" href="javascript:void(0)" onclick="detail(\''+ v.id+'\')">审核</a></td>';
        html +='</tr>' ;
