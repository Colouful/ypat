  ...
  >
    ...
    >
      <view
        ...
        :style="{color:currentTab==index?selectedColor:color,fontSize:selectedSize&&currentTab==index?selectedSize+'rpx':size+'rpx',lineHeight:size+'rpx',fontWeight:bold && currentTab==index?'bold':'normal'}"
      >{{item.name}}</view>
    </view>
    <view
      class="tui-tabs-slider"
      :style="{transform:'translateX('+scrollLeft+'px)',width:sliderWidth+'rpx',height:
	sliderHeight+'rpx',bottom:bottom+'rpx',background:sliderBgColor}"
    ></view>
  </view>
</template>
