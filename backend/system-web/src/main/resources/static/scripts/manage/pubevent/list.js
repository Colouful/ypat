...
var pageaction = function(){
    $.get('./list?t='+new Date().getTime(),{
        dateStrStart:$("#dateStrStart").val(),
        dateStrEnd:$("#dateStrEnd").val(),
...
function fillData(data){
    ...
    $.each(data,function(k,v){
        ...
        html += '<tr> ' +
            ...
            '<td>'+ (v.dateStr==null?'':v.dateStr) +'</td>' +
            '<td>'+ (v.eventKey==null?'':v.eventKey) +'</td>' +
            '<td>'+ (v.eventKey==null?'':v.eventKeyTxt) +'</td>' +
            '<td>'+ (v.msgTimes==null?'':v.msgTimes) +'</td>';
        html +='</tr>' ;
        $list.append($(html));
    });
}
