...
| 编号 | 级别 | 说明 | 处理 |
|---|---|---|---|
| GAP-G-01 | P1 | 仅 2 张证件照(缺手持),旧需 3 张且有 29 元审核费 | 后端和管理后台兼容两照;已创建 ADR,当前采用两照免费认证,状态 ACCEPTED_WITH_ADR |
...
- 结论: 实名核心流程可用且隐私采集更少;GAP-G-01 已通过 ADR 接受两照免费策略;GAP-IMG-01 和 dataURL 后端兼容已修复。
